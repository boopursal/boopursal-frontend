...
function SuggestionsApp(props) {
    ...
    useEffect(() => {
        if (!user.id)
            return
        dispatch(Actions.getSuggestions(user.id));
    }, [dispatch, user]);
    ...
    return (
        <React.Fragment>
            <Helmet>
                <title>Demande de Suggestion | Les Achats Industriels</title>
            ...
            <FusePageSimple
                ...
                header={
                    <SuggestionsHeader pageLayout={pageLayout} />
                }
                content={
                    <SuggestionsList />
                }
...
export default withReducer('SuggestionsApp', reducer)(SuggestionsApp);
